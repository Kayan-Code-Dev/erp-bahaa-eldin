...
<body>
    <div class="card">
        <h1>تفعيل حساب مدير الفرع</h1>

        <p>أهلاً <strong>{{ $branchManager->first_name ?? $branchManager->name }}</strong>، نحتاج منك إدخال رمز التفعيل
            لإكمال
            تفعيل الحساب.</p>

        <form method="POST" action="{{ route('branchManager.activate.verify', $branchManager->uuid) }}">
            @csrf
            <div class="form-row">
